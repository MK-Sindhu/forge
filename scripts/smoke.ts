...
async function main() {
  ...
  const { users, worlds, likes, worldMedia, follows, comments, reposts, worldUpdates, reports } = await import("../src/db/schema");
  ...
  const rp = await db.select().from(reports).limit(1);
