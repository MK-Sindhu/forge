...
└── components/               # Shared React components (each in its own kebab-case directory)
    ...
    │   ├── EditUsernameButton.tsx         # "use client"; hotfix; props: currentUsername, isOwnProfile; renders null when not own profile; pencil icon button that opens EditUsernameDialog; inline SVG, no icon library
    │   └── EditUsernameDialog.tsx         # "use client"; hotfix; props: currentUsername, open, onClose; native <dialog> modal; PUT /api/users/me with { username }; 200 → onClose() + router.push('/profile/<newUsername>'); 400/409 → inline error; warning text about broken links; state resets via requestAnimationFrame callbacks (not effect body, satisfies react-hooks/set-state-in-effect); aria-modal + aria-labelledby + aria-describedby; no new deps
    ├── world-visitor/
