...
**Last updated:** 2026-05-27 (10.1 close)
**Current phase:** Slice 10 — Realtime editor collab (the Phase 3 work). 10.1 shipped 2026-05-27. 10.2 (concurrent CRDT-based co-editing) + 10.3 (voice chat via LiveKit) planned. Phase 1 launch ops still deferred per founder; see `DEFERRED.md`.
**Current slice:** Slice 10.1 (realtime editor presence + chat-in-editor + rebase toast) shipped 🟢. Builds on Slice 9 (visitor walk + collab + presence + chat).
...
| Metric | Value |
|---|---|
| Slices shipped | 0, 1, 2, 3, 4, 5, 6, 7 ✅ · Phase 2 (8.1 + 8.2 + 8.3 + 8.4) 🟢 · Slice 9 (9.1 + 9.2 + 9.3) 🟢 · Slice 10.1 🟢 |
| Phase 1 launch ops | Deferred per founder — see `DEFERRED.md` |
| Tests passing | 822 across 57 test files |
| Commits on main | Slices 0–7 + Phase 2 + Slice 9 + Slice 10.1 closeout (latest commit pending) |
...
- **Slice 10.1 — Realtime editor presence (Phase 3 work, first sub-slice of Slice 10).** When 2+ editors open `/world/[id]/edit` on the same world, they see each other live: cursor positions as colored 3D spheres + name tags, selection outlines on the OTHER editor's selected object, "X other editor(s) here" text in the status bar, avatar stack in the top bar (up to 4 + overflow pill), in-editor chat panel (same `ChatPanel` from 9.3), and a rebase-toast banner when autosave merges another editor's changes. Single Liveblocks room per world means visitors + editors share presence; each surface filters by `mode` to keep visual clutter down. Refactored `VisitorPresence` into a discriminated union `UserPresence = VisitorPresence | EditorPresence` with `mode: "visitor" | "editor"` discriminant. New `EditorPresence` shape: `{ mode, cursorWorldPos, selectedObjectId, gizmoMode }` (camera position omitted v1). `LiveblocksRoomProvider` now requires `initialPresence: UserPresence` as a prop (caller pages pass the right initial shape). New components: `EditorPresenceLayer` (R3F filter+map), `RemoteEditorCursor` (sphere + Billboard name tag), `RemoteEditorSelectionOutline` (Box3-traverse fallback for finding remote-selected objects + wireframe box), `EditorCollaborators` (top-bar avatar stack), `RebaseToast` (bottom-center floating pill, 5s auto-dismiss). New hook `useEditorPresence` (pure helper `computeEditorPresence` + 100ms throttle + serialized-state dedup). `EditorAssetMesh` extended with `userData.objectId` tag. `editor-store` extended with `lastRebaseNotice` + `setRebaseNotice`. `use-autosave` calls `setRebaseNotice` on the 409→rebase branch. Concurrent CRDT-based co-editing remains out of scope (Slice 10.2). 799 → 822 tests (+23). NO new deps, NO schema, NO env vars. Pure presence layer atop Slice 9.3.
...
- **2026-05-27** — Slice 10.1 plan-time decisions (Realtime editor presence): (a) **Single Liveblocks room per world** for visitors + editors (the existing `world:${worldId}`). Each surface filters `useOthers()` by `mode` to control visual clutter. Cheap to switch to two rooms later if overlap proves problematic — but the founder explicitly wants the "community in one world" feel. (b) **Discriminated union presence shape** — `UserPresence = VisitorPresence | EditorPresence` with `mode: "visitor" | "editor"` discriminant. Future modes (e.g., `mode: "spectator"`) plug in cleanly. Type-safe at compile time via `isWalkingVisitor()` + `isEditor()` guards. (c) **`LiveblocksRoomProvider` accepts `initialPresence` as a required prop** instead of hardcoding visitor shape — minimal change preserving the single provider. Visitor page passes `INITIAL_VISITOR_PRESENCE`; editor page passes `INITIAL_EDITOR_PRESENCE`. (d) **Editor presence fields = `cursorWorldPos` + `selectedObjectId` + `gizmoMode`** (no camera position in v1 — adds when a consumer like camera-frustum visualization ships). (e) **Editor → visitor visibility:** walking visitors in the same world DO appear in editor's `useOthers()` but `EditorPresenceLayer` filters by `mode === "editor"` (editing is heads-down work; capsules walking through your scene = distracting). (f) **Visitor → editor visibility:** editors filtered out of `PresenceLayer` because editor presence has no `inWalkMode` field — naturally excluded by `isWalkingVisitor` guard. (g) **Chat = same room, same panel** — drop `<ChatPanel />` from 9.3 into `EditorShell`. Editors and visitors in the same world chat together. The "one community per world" call. (h) **Save-conflict UX = rebase toast** — bottom-center floating pill "Another editor's changes were merged in — your edits applied on top." Without this the rebase is silent + feels buggy. Auto-dismiss 5s. (i) **Author name in rebase toast = OMITTED v1** — would require extending the 409 response's `currentVersion` shape with `with: { author: true }` on the version row query. Small but additional surface; TODO comment + parking lot. (j) **Camera frustum visualization for remote editors = DEFERRED** to a follow-up. Visual clutter risk with 3+ editors; verify need from real use. (k) **T-key conflict** between editor's gizmo-mode shortcut + chat's "focus chat" shortcut: kept as-is. Pressing T outside any input both flips gizmo to translate AND focuses chat input. Side effect is harmless; could swap chat to `/` later if it bothers users. (l) **No new schema/migration/env vars/deps** — pure layer atop 9.3's infrastructure. (m) **Pure-helper extraction continues**: `computeEditorPresence` exposed for testability — real Three.js instances (`PerspectiveCamera`, `Scene`, `Raycaster`) work in the `node` Vitest env (no WebGL needed for math). (n) **R3F-Canvas-wrapping components NOT unit-tested** — consistent with Phase 2 + Slice 9 precedent. `RebaseToast` also skipped (effect-heavy, requires JSDOM); store-level coverage is sufficient.
