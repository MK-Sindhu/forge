...
**Current slice:** Phase 2 (8.1–8.4) + Slice 9.1 (walk mode + collision + copy reframe) + Slice 9.2 (collaborators) all shipped 2026-05-26 🟢. **9.3 (multi-user presence + chat via Liveblocks) next.**
...
| Metric | Value |
|---|---|
| Slices shipped | 0, 1, 2, 3, 4, 5, 6, 7 ✅ · Phase 2 (8.1 + 8.2 + 8.3 + 8.4) 🟢 · Slice 9.1 + 9.2 🟢 |
| Phase 1 launch ops | Deferred per founder — see `DEFERRED.md` |
| Tests passing | 761 across 51 test files |
| Commits on main | Slices 0–7 + Phase 2 + Slice 9.1 + 9.2 closeout (latest commit pending) |
...
| Slices verified in prod | 1, 3, 6, 7 ✅ — 2, 4, 5, 8.1, 8.2, 8.3, 8.4, 9.1, 9.2 deployed but not yet prod-smoked. Prod has 0010 + 0011 applied (2026-05-26); 0012 prod migration pending. |
| In-flight | Awaiting founder prod migration for 0012 (9.2 collaborators schema); then prod smoke 9.1 + 9.2 + Phase 2; then 9.3 (Liveblocks presence + chat). |
...
4. Phase 2 (8.1 + 8.2 + 8.3 + 8.4) + Slice 9.1 + Slice 9.2 are shipped 🟢. Next:
   - **Founder action:** prod migration for 0012 (`DATABASE_URL=... npm run db:migrate` against the Neon prod URL). Adds `world_collaborators` table + extends `notifications.type` CHECK. 0010 + 0011 already applied 2026-05-26.
   - **Then:** prod smoke the full stack — convert a world → open in editor → invite a friend as collaborator → friend uses editor → publish → walk around with collision. Flip 🟢 → ✅ once green.
