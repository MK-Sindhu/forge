...
**Current slice:** Sub-slices 8.1 + 8.2 + 8.3 shipped 2026-05-26. 8.5 absorbed into 8.3. **8.4 (browser editor) is next — the headline visible feature.**
...
| Metric | Value |
|---|---|
| Slices shipped | 0, 1, 2, 3, 4, 5, 6, 7 ✅ · Phase 2 sub-slices 8.1 + 8.2 + 8.3 🟢 (8.5 absorbed into 8.3) |
| Phase 1 launch ops | Deferred per founder — see `DEFERRED.md` |
| Tests passing | 542 across 33 test files |
| Commits on main | Slices 0–7 + Phase 2 8.1 + 8.2 + 8.3 closeout (latest commit pending) |
...
| Slices verified in prod | 1, 3, 6, 7 ✅ — 2, 4, 5, 8.1, 8.2, 8.3 deployed but not yet prod-smoked |
| In-flight | Awaiting founder prod migration for 0010 + 0011 (8.1 + 8.2 schema); then prod smoke of 8.3 visible UI (convert button + version history) + folder-watcher CLI; then 8.4 (browser editor). |
...
4. Phase 2 sub-slices 8.1 + 8.2 + 8.3 are shipped 🟢. Next:
   - **Founder action:** prod migrations for 0010 + 0011 (`DATABASE_URL=... npm run db:migrate` against the Neon prod URL). Note: 8.3 added NO new migrations — pure code + UI on top of 8.1/8.2 schema. So one migration step covers all three sub-slices.
   - **Then:** prod smoke the 8.3 visible UI per the TRACKER 10-check list — convert a legacy world, see version history, optionally run the folder-watcher CLI. Flip 🟢 → ✅ once green.
   - **Then:** Sub-slice 8.4 (browser editor) — the headline visible feature; first real client of the 8.2 API. 8.5 is absorbed into 8.3.
